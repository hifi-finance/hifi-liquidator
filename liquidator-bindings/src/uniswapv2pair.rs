...
mod uniswapv2pair_mod {
    ...
    impl<'a, M: Middleware> UniswapV2Pair<M> {
        ...
        pub fn swap(&self, amount_0_out: U256, amount_1_out: U256, to: Address, data: Vec<u8>) -> ContractCall<M, ()> {
            self.0
    ...
    impl MintFilter {
        ...
        pub const fn signature() -> H256 {
            H256([
                76, 32, 155, 95, 200, 173, 80, 117, 143, 19, 226, 225, 8, 139, 165, 106, 86, 13, 255, 105, 10, 28, 111,
                239, 38, 57, 79, 76, 3, 130, 28, 79,
            ])
        }
    ...
    impl Detokenize for MintFilter {
        fn from_tokens(tokens: Vec<Token>) -> Result<Self, InvalidOutputType> {
            ...
            let amount_0 = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            let amount_1 = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            Ok(MintFilter {
                sender,
                amount_0,
                amount_1,
    ...
    impl Detokenize for ApprovalFilter {
        fn from_tokens(tokens: Vec<Token>) -> Result<Self, InvalidOutputType> {
            ...
            let value = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            Ok(ApprovalFilter { owner, spender, value })
        }
    }
    #[derive(Clone, Debug, Default, Eq, PartialEq)]
    ...
    impl BurnFilter {
        ...
        pub const fn signature() -> H256 {
            H256([
                220, 205, 65, 47, 11, 18, 82, 129, 156, 177, 253, 51, 11, 147, 34, 76, 164, 38, 18, 137, 43, 179, 244,
                247, 137, 151, 110, 109, 129, 147, 100, 150,
            ])
        }
        #[doc = r" Retrieves the ABI signature for the event this data corresponds"]
        #[doc = r" to. For this event the value should always be:"]
    ...
    impl SwapFilter {
        ...
        pub const fn signature() -> H256 {
            H256([
                215, 138, 217, 95, 164, 108, 153, 75, 101, 81, 208, 218, 133, 252, 39, 95, 230, 19, 206, 55, 101, 127,
                184, 213, 227, 209, 48, 132, 1, 89, 216, 34,
            ])
        }
        #[doc = r" Retrieves the ABI signature for the event this data corresponds"]
        #[doc = r" to. For this event the value should always be:"]
    ...
    impl Detokenize for SwapFilter {
        fn from_tokens(tokens: Vec<Token>) -> Result<Self, InvalidOutputType> {
            ...
            let mut tokens = tokens.into_iter();
            let sender = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            let amount_0_in = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            let amount_1_in = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            let amount_0_out = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            let amount_1_out = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            let to = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            Ok(SwapFilter {
                sender,
                amount_0_in,
    ...
    impl SyncFilter {
        ...
        pub const fn signature() -> H256 {
            H256([
                28, 65, 30, 154, 150, 224, 113, 36, 28, 47, 33, 247, 114, 107, 23, 174, 137, 227, 202, 180, 199, 139,
                229, 14, 6, 43, 3, 169, 255, 251, 186, 209,
            ])
        }
        #[doc = r" Retrieves the ABI signature for the event this data corresponds"]
        #[doc = r" to. For this event the value should always be:"]
    ...
    impl Detokenize for SyncFilter {
        fn from_tokens(tokens: Vec<Token>) -> Result<Self, InvalidOutputType> {
            ...
            #[allow(unused_mut)]
            let mut tokens = tokens.into_iter();
            let reserve_0 = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            let reserve_1 = Tokenizable::from_token(tokens.next().expect("this should never happen"))?;
            Ok(SyncFilter { reserve_0, reserve_1 })
        }
    }
}
